...
describe('useTableSort Hook', () => {
  ...
  describe('Valores edge cases', () => {
    ...
    it('debería manejar strings con números', () => {
      ...
      const { result } = renderHook(() => useTableSort(mixedData))

      act(() => {
        result.current.handleSort('code')
      })

      // El ordenamiento alfabético de strings da: '1', '10', '2'
      expect(result.current.sortedData[0].code).toBe('1')
      expect(result.current.sortedData[1].code).toBe('10')
      expect(result.current.sortedData[2].code).toBe('2')
    })
  })
})